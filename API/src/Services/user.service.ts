import { CodigoHTTP, MensajeHTTP } from "../Utilis/codigos.http";
import { usersData } from "../Data/users";
import IUserService from "../Interfaces/user.service.interface";
import IUserModel from "../Models/user.model";
import { ErrorHandler } from "../Utilis/error.handler";
import { isNotEmpty } from "../Utilis/isEmpty";
import { IResponseHandler, ResponseHandler } from "../Utilis/response.handler";
...
export class UserService implements IUserService {
  ...
  DeleteUser(guid: string): IResponseHandler<IUserModel> {
    ...
    if(find){
      ...
      return ResponseHandler<IUserModel>(MensajeHTTP.Deleted);
    }
    throw ErrorHandler(CodigoHTTP.NotFound);
  }
}
